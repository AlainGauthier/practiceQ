...
if(mysqli_num_rows($query) > 0){
	...
	while($row = $query->fetch_assoc()) {
     ...
     $sql = "INSERT INTO user (username, avatar, email, password ) VALUES ('".$username."', '".$avatar."', '".$email."', '".$password ."')";
     if ($con->query($sql) === TRUE){
	     $extToken = $_POST['token'];
	     $token = bin2hex(openssl_random_pseudo_bytes(16));
	     $userId = $con->insert_id;
	     $sql = "INSERT INTO token (userId, token, ext_token ) VALUES ('".$userId."', '".$token."', '".$extToken."')";
	     if ($con->query($sql) === TRUE)
    	      echo json_encode( ['token' => $token, 'username' =>  $username, 'avatar' => $avatar, 'email' =>  $email, 'id' => $userId ] );
    	 else {
        	  echo json_encode( ['err' => "SOMETHING WRONG!, WE'RE CHECKING IT!!"]);   
     	 }
